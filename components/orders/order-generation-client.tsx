...
export function OrderGenerationClient({ contacts }: { contacts: Contacts }) {
  ...
  return <main className="order-page">
    <header className="order-header"><Link href={catalogHref}><Image src="/ondie-logo.svg" alt="ONDIE" width={176} height={58} priority /></Link><label><MagnifyingGlass /><input aria-label="Buscar productos o tiendas" placeholder={exclusiveStore ? `Buscar en ${exclusiveStore.storeName}...` : "Buscar productos o tiendas..."} /></label>{!exclusiveStore ? <nav><Link href="/">Tiendas</Link></nav> : null}<div><CartSummary /><Link href="/account" aria-label="Mi cuenta"><UserCircle /></Link></div></header>
