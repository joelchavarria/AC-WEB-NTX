...
import { ArrowRight, CheckCircle, Clock, Heart, MagnifyingGlass, MapPin, Package, ShareNetwork, ShieldCheck, SlidersHorizontal, SortAscending, Storefront, Truck, UserCircle, WhatsappLogo, X } from "@phosphor-icons/react";
...
export function StorefrontClient({ store, exclusive = false }: { store: Store; exclusive?: boolean }) {
  ...
  const coverImage = store.store_json?.heroImage ?? store.store_json?.profile_settings?.coverImage ?? null;
  const businessHours = store.store_json?.profile_settings?.hours;
