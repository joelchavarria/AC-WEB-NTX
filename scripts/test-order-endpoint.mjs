...
const body = {
  customer: { name: "Cliente", phone: "88888888", address: "Managua" },
  paymentMethod: "cash",
  deliveryMethod: "store_delivery",
  groups: [
    {
      storeId,
      deliveryOptionId: "managua",
      items: [{ id: productId, quantity: 1, price: 0.01, name: "Forged" }],
    },
  ],
};
    ...
    return {
      createClient: () => ({
        rpc: async (name, args) => {
          ...
          if (name === "consume_checkout_rate_limits")
            return { data: limit, error: null };
          return {
            data: [{ id: productId, orderNumber: 1, storeId }],
            error: failure,
          };
        },
      }),
...
function request(value = body, headers = {}) {
  return new Request("https://ondie.test/api/orders", {
    method: "POST",
    headers: {
...
assert.equal(
  (await exports.POST(request(body, { "Idempotency-Key": "invalid" }))).status,
  400,
);
assert.equal(
  (await exports.POST(request(body, { Origin: "https://attacker.test" })))
    .status,
  403,
);
assert.equal(
  (await exports.POST(request(body, { "Sec-Fetch-Site": "cross-site" })))
    .status,
  403,
);
assert.equal(
  (await exports.POST(request(body, { "Content-Type": "text/plain" }))).status,
  415,
);
...
bad.groups[0].items[0].quantity = -1;
assert.equal((await exports.POST(request(bad))).status, 400);
assert.equal(
  calls.length,
  0,
  "Invalid requests must not call privileged database functions",
);
const bot = structuredClone(body);
bot.website = "https://bot.invalid";
assert.equal((await exports.POST(request(bot))).status, 403);
assert.equal(calls[0].name, "record_checkout_security_event");
calls = [];
...
assert.ok(
  calls[0].args.p_client_keys.every((key) => /^[0-9a-f]{64}$/.test(key)),
);
limit = false;
...
console.log(
  "PASS: endpoint validation, origin, hard body limit, idempotency key, canonical payload, rate limiting and sanitized database errors",
);
